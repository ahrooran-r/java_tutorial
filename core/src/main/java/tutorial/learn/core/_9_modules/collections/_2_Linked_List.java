...
public class _2_Linked_List {
    public static void main(String[] args) {
        ...
        // creating empty linked list, specify initial capacity
        LinkedList<String> alphabet = new LinkedList<>();

        // add element
        alphabet.add("Banana");
        alphabet.addFirst("Car");
        alphabet.addLast("Tree");

        // add element at specific location
        alphabet.add(1, "Orange");

        // add all elements of another list in 2nd position
        alphabet.addAll(2, Arrays.asList(ALPHABET));

        // view elements
        alphabet.toString(); // -> [Car, Orange, A, B, C, ... X, Y, Z, Banana, Tree]

        // get an element
        alphabet.get(5); // -> D

        // change an element
        alphabet.set(0, "Apple"); // -> [Apple, Orange, .... ]

        // removes and returns element
        alphabet.remove(25); // -> [Apple, B, ..... Y]
        // removes an object in list and returns  true/false
        alphabet.remove("B"); // -> true
        alphabet.removeFirst();
        alphabet.removeLast();

        // Finding elements in the linked list
        alphabet.contains("E"); // -> true
